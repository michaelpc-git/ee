...
// WP-Optimize advanced-cache.php (written by version: 4.1.1) (homeurl: http://localhost:10013/) (abspath: /home/com1os/Local Sites/eroding-empire-2025/app/public/) (do not change this line, it is used for correctness checks)
...
$possible_plugin_locations = array(
	...
	'/home/com1os/Local Sites/eroding-empire-2025/app/public/wp-content/plugins/wp-optimize/cache',
);
...
if (false === $plugin_location) {
	...
	if (strcasecmp('http://localhost:10013/', $protocol . '://' . $host . $request_uri) === 0) {
...
if (false !== $plugin_location) {
	...
	if (!defined('WPO_CACHE_EXT_DIR')) define('WPO_CACHE_EXT_DIR', '/home/com1os/Local Sites/eroding-empire-2025/app/public/wp-content/plugins/wp-optimize/cache/extensions');
}

if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-localhost-port10013.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-localhost-port10013.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-localhost-port10013.php');
}
