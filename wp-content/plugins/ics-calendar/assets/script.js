...
function r34ics_ajax_init(elem) {
	jQuery(document).trigger('r34ics_ajax_start', [elem]);

	jQuery(document).find('.r34ics-ajax-container').each(function() {
		r34ics_ajax_request(jQuery(this), 0);
	});

	jQuery(document).trigger('r34ics_ajax_end', [elem]);
}


function r34ics_ajax_request(r34ics_elem, deprecated) {
	r34ics_elem.html('');
...
function r34ics_init(elem) {
	...
	jQuery(document).on('keypress', '.ics-calendar *, .r34ics_lightbox *', function(e) {
		if (e.which == 13) {
			...
			if (jQuery(this).hasClass('ics-calendar-paginate') && jQuery(this).hasClass('prev')) {
				jQuery(document).find('.ics-calendar-paginate.prev:visible').focus();
			}
			else if (jQuery(this).hasClass('ics-calendar-paginate') && jQuery(this).hasClass('next')) {
				jQuery(document).find('.ics-calendar-paginate.next:visible').focus();
			}
		}
	});
	...
	*/
	jQuery(document).on('click', '.ics-calendar.r34ics_toggle .event, .ics-calendar.toggle .event, .ics-calendar.r34ics_toggle .event .title, .ics-calendar.toggle .event .title', function(e) {
		...
		if (jQuery('.r34ics_lightbox').length > 0 && elem.closest('.ics-calendar').hasClass('r34ics_toggle_lightbox')) {
			...
			jQuery('.r34ics_lightbox .r34ics_lightbox_content .descloc_feed_label').css('border-color', elem.attr('data-feed-color'));
	...
	jQuery('.ics-calendar:not(.sametab) a').each(function() {
		if (
			...
			jQuery(this).closest('.ics-calendar').hasClass('newtab')
		) {
	...
	if (jQuery('.ics-calendar.layout-week').length > 0) {
		...
		jQuery('a[data-ics-calendar-action="show-past-events"]').on('click', function() {
			...
			// Make sure we're showing all of the proper headers
			r34ics_show_hide_headers();
			// Don't jump!
			return false;
		});
		...
		jQuery('.ics-calendar.layout-month .ics-calendar-select').on('change', function() {
			...
			if (r34ics_arrownav.length > 0) {
				...
				if (r34ics_arrownav_prev.length > 0) {
					r34ics_arrownav.find('.prev').data('goto', r34ics_arrownav_prev.attr('value')).removeClass('inactive').removeAttr('aria-hidden');
					r34ics_arrownav.find('.prev-text').text(r34ics_arrownav_prev.text());
				}
				else {
					r34ics_arrownav.find('.prev').data('goto', '').addClass('inactive').attr('aria-hidden', 'true');
				...
				if (r34ics_arrownav_next.length > 0) {
					r34ics_arrownav.find('.next').data('goto', r34ics_arrownav_next.attr('value')).removeClass('inactive').removeAttr('aria-hidden');
					r34ics_arrownav.find('.next-text').text(r34ics_arrownav_next.text());
				}
				else {
					r34ics_arrownav.find('.next').data('goto', '').addClass('inactive').attr('aria-hidden', 'true');
				...
				if (r34ics_arrownav_current.val() != r34ics_arrownav.find('.today').data('goto')) {
					r34ics_arrownav.find('.today').removeClass('inactive').removeAttr('aria-hidden');
				}
				else {
					r34ics_arrownav.find('.today').addClass('inactive').attr('aria-hidden', 'true');
				}
			}
		});
